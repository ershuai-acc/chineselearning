...
export default function PinyinLessonPage() {
  ...
  if (!content) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[100dvh] p-6">
        <p className="text-xl font-bold text-gray-600">课程未找到</p>
        <button onClick={() => router.back()} className="mt-4 text-duo-macaw font-bold">返回</button>
      </div>
    );
  }
  ...
  // Derive albumId from lessonId for back navigation
  ...
  function generatePracticeQuestions() {
    return words.map(word => {
      ...
      const distractor = others.length > 0
        ...
        : word;
      const options = [word, distractor].sort(() => Math.random() - 0.5);
  ...
  if (phase === 'learn') {
    ...
    return (
      <div className="flex flex-col min-h-[100dvh] bg-duo-snow">
        {/* Header */}
        <div className="flex items-center gap-4 p-4 sticky top-0 bg-duo-snow z-10">
        ...
        {/* Bottom */}
        <div className="p-4 border-t-2 border-duo-swan bg-white sticky bottom-0">
          <button onClick={handleNextLearn} className="w-full py-4 bg-duo-green text-white font-extrabold text-lg rounded-2xl border-b-4 border-[#58A700] active:border-b-0 active:translate-y-1 transition-all">
            {learnIndex === words.length - 1 ? '开始练习' : '继续'}
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex flex-col min-h-[100dvh] bg-duo-snow">
        ...
        <div className="flex-1 flex flex-col px-4 pt-4">
          <h2 className="text-xl font-extrabold text-duo-eel mb-6">你听到了什么？</h2>
        ...
        {/* Feedback / Check */}
        {showFeedback ? (
          <div className={`p-6 ${isCorrect ? 'bg-green-50' : 'bg-red-50'} border-t-2 ${isCorrect ? 'border-green-200' : 'border-red-200'}`}>
  ...
  // COMPLETE PHASE
  return (
    <div className="flex flex-col min-h-[100dvh] bg-duo-snow items-center justify-center text-center p-6">
      <div className="text-6xl mb-6 animate-bounce">🎉</div>
      <h2 className="text-3xl font-extrabold text-duo-green mb-4">太棒了!</h2>
      <p className="text-lg text-duo-hare mb-8">你已完成本课学习</p>
